...
- Scopus with AI (owner directed 2026-07-08): the site's first institutionally licensed resource. AUA's institution-wide Scopus license (arranged through the AUA Library, live July 2026; the owner initiated the procurement and vetted the AI layer before it, so the entry carries `governance_status: approved` by owner direction, the directory's first) explicitly includes Scopus AI, confirmed by the owner in the live product. Listed in the tools directory's Research category with the library proxy sign-in link as its URL (tested HTTP 200; Elsevier's product page is the verify_url since the proxy serves only a login page), cost chip "AUA-licensed", and a dedicated section atop the research guide covering the feature set verified from Elsevier's live page 2026-07-08 (cited summaries with confidence indicators, concept maps, foundational papers, emerging themes, agentic Deep Research) and the grounding caveats stated plainly (abstracts and metadata rather than full text, coverage concentrated from 2003 onward, summaries not citable per Elsevier). Also added to the research pipeline figure's find-and-map stage and the cost table. Timing was coordinated with the Library's own announcement window; the site was not yet advertised beyond the AI Committee when this shipped. Editorial stance (owner directive 2026-07-08): the research guide is Scopus-first where Scopus is genuinely competitive (search, orientation, citation chasing, and a first pass at evidence questions), because a licensed, approved tool is the correct default over freemium alternatives at parity; recommendations are never skewed where another tool is plainly better for the task (structured screening stays Elicit, own-source synthesis stays NotebookLM, citation-context verdicts stay Scite, point-of-care stays OpenEvidence, seed-paper visual mapping stays ResearchRabbit). Future content passes preserve this stance.
...
- Opportunities section (owner approved 2026-07-06, prompted by a dean forwarding a buildathon invitation by email): data/opportunities.yaml (owner-owned; schema, scope, and vetting bar documented in the file header) renders through its own marker onto docs/opportunities.md, a top-level nav entry after Conferences, cross-linked from the Conferences page and the For Students take-part section. Scope is global by owner directive: any opportunity AUA faculty or students would be eligible for that is relevant to AI in medicine or the fields around it. Vetting bar per entry: official page reachable, organizer identifiable, free to enter, deadline confirmed at source on the `verified` date; support and prize figures always framed as organizer-stated; canonical URLs only, never shortened links. Date deadlines get the conference-style countdown badge and entries archive automatically once the event ends (or a dated deadline passes for entries without event dates). A weekly opportunity-watch workflow (Mondays, scripts/opportunity_watch.py, claude-haiku-4-5, pennies per run) screens verified fetchable sources (Devpost online hackathons and grand-challenge.org's API; MLH, PhysioNet, NIH Guide, and Kaggle were probed 2026-07-06 and rejected, reasons recorded in feeds.yaml) for relevance and international eligibility, and proposes keeps as GitHub issues with paste-ready YAML skeletons. Candidates are screened exactly once (data/opportunity_watch_state.json, pipeline-owned). The word-of-mouth stream is deliberately human: the page invites forwarded emails via the feedback form. Inaugural entry: Future Caribbean Global Agentic AI Buildathon, verified 2026-07-06 (official site live, June 2026 press coverage, named founder and partners, free entry, healthcare track; the emailed bit.ly links were replaced with the canonical site). New listings and redated entries flow into the weekly digest's "New opportunities" section (snapshot diff in the ledger, mirroring the conference mechanism; the first digest deliberately announces everything currently open because the section is newer than its first digest).
- Tiered autonomy (owner directive 2026-07-08, superseding the original propose-only-always stances, to minimize daily management): watches may apply their own findings when, and only when, every mechanical gate passes; judgment stays human. Opportunity watch: keeps hosted on managed platforms (Devpost, grand-challenge, where the platform vets organizers) go through a gate_verify pass (claude-sonnet-5) that extracts a structured verdict from the fetched official page and auto-apply only when the page confirms a current opportunity, the organizer is named, free entry is confirmed, the deadline is a future date or rolling, the format is confirmed, and the name is not already listed; unknown domains and gate failures stay proposals. Content watch: open-weights model adds auto-apply only when gate_verify identifies the official Hugging Face repository from search results against the news evidence AND the card's license tag is apache-2.0 or mit; any other tag (including "other", which hides custom terms like TabFM's non-commercial license) stays a proposal for a human license read, as do all tool changes and all removals. Every auto-application lands with an inline auto-verified comment and a chore(auto) commit, which is the audit trail; each watch's issue carries an Auto-applied section when an issue opens, and auto-applications alone never open an issue (issues are exceptions-only by design). Kill switches: `opportunity_watch.mode: propose` and `content_watch.models_add_mode: propose` in feeds.yaml. Floors that stay human regardless: governance statuses and the review rubric, the policy page, prompt-library review statuses, all removals, and everything in tools.yaml beyond last_reviewed bumps. Phase 3 (auto-adding new conferences and new tools under equivalent gates) is queued pending a few weeks of clean live behavior from these two.
- Prose page review, census-derived freshness budgets (owner approved 2026-07-08, extending the tiered-autonomy doctrine to hand-authored prose, the last unwatched surface): a docs page carrying `last_reviewed` front matter is enrolled (24 pages enrolled 2026-07-08 with dates from each page's last substantive commit; docs/news, docs/governance, and docs/announcements are excluded by construction). Each page's review budget is computed from its own volatile-claim census, never a preset tier: budget_days = 30 x target_expected_drift / sum(claims_per_class x lambda_per_month), clamped to [45, 365] days; claim classes are named models/prices/specs, dated phrasings, and live directory names matched against tools.yaml plus open_models.yaml, so budgets tighten automatically as a page cites more watched tools. The cold-start lambda rates in feeds.yaml `content_watch.page_review` are grounded in the first month of observed drift (content-watch proposal rates over the roughly 40-entry directory; the research page needing a substantive rewrite within four weeks of authoring); costs are asymmetric (a review costs about a cent, a stale claim sits on an institutional site), so estimates round down. At enrollment the hardware and research pages floor at 45 days, agents at 50, local at 125, and the concept, pathway, and playbook pages sit at the 365 cap; nothing is due before 2026-08-16. Weekly pass (third input of content_watch.py, logic and doctrine in scripts/page_review.py): up to max_pages_per_run due pages get a claims check by the llm.tasks page_review model (claude-sonnet-5), grounded in text fetched that day from the mentioned tools' verify pages; claims must be quoted from the page, tool pages are evidence only. A clean review bumps the page's front-matter `last_reviewed` line, the only edit the pipeline ever makes inside a hand-authored page (field-level exception mirroring the tools.yaml rule). Drift never edits prose: it must persist across two consecutive reviews (the conference-watch stability gate) and then escalates into the content-watch issue quoting the exact sentences with evidence. An optional `review_by` front-matter date forces review on a known date, since scheduled expirations beat statistical budgets. Tool-mention tripwire: content-watch recommendations append the enrolled prose pages naming the affected entry, so prose is rechecked the week a tool's facts move rather than when its own budget expires; this event channel, not the budgets, is expected to catch most drift. data/page_review_state.json (pipeline-owned) holds the pending-drift flags and a drift ledger (reviews, claims checked, drifted, unverifiable) so the lambda rates can be re-estimated from observation as reviews accumulate. Kill switch `content_watch.page_review.mode: off`; force one page through review via the content-watch workflow's `review_page` dispatch input or locally with `--review-page <path relative to docs/>` (claim verdicts print in the run log). End-to-end verified 2026-07-08: forced review of the hardware page enumerated 12 claims from page text (bandwidth hierarchy, quantization arithmetic, tool pricing) and judged them current.
- Pre-launch navigation and landing polish (owner approved 2026-07-09, from a fresh-eyes audit at 1280/1440/375px in both schemes; goal: an exploring-friendly site, not walls of text): the homepage hides both sidebars (front matter) for a full-bleed landing with the hero and three-up card grid; a dismissible announcement banner renders from the announce block in overrides/main.html (announce.dismiss feature; Material remembers dismissal per visitor until the text changes; edit or empty the block to change or retire it; banner links use the accent color via extra.css because the brand-blue primary measured ~2.4:1 on the dark strip; banner URLs must be page-independent, via config.site_url, never the page-relative url filter: dismissal is keyed to a hash of the banner's rendered content, and a relative href made the hash differ per page so the banner reappeared on every page until dismissed there too, owner-reported and fixed 2026-07-09); navigation.tabs.sticky keeps section tabs visible when scrolling long pages; toc.follow keeps the contents column tracking the reader; navigation.indexes makes the Learn, Tools, Prompts, and Playbooks section names clickable to their overview pages, absorbing the former Start Here/Directory/Library/Overview child entries (URLs unchanged); the tools landing H1 is "Tool Directory" to match its nav label. Deliberately rejected in the same audit: navigation.instant (breaks the three custom scripts unless rewritten against Material's page observable; wrong risk the week before launch), restructuring the 13 top-level sections (they fit above the 1220px tab breakpoint, collapse to the drawer below it by design, and URL churn before announcement links go out is the wrong trade; revisit post-launch with analytics), and the social-cards plugin (a hand-built og:image card already ships in overrides/main.html). The audit also verified: mobile hero, dark-scheme figure contrast, tab fit at 1280px, and complete Open Graph metadata.
- External-review package (owner approved 2026-07-09; an outside design review was verified claim by claim against the repo before acting, and its stale claims discarded): homepage promises now match content maturity (hero "find the right tools"; Tools card "curated directory ... visible governance status"; Prompts card names the Draft or Reviewed statuses and no longer advertises the empty community exchange), since 54 of 55 tools were Under review and 5 of 6 prompts Draft while the homepage said "reviewed" and "tested". The prompt-maturity admonition is retitled "About the linked prompts" (the "Under construction" title branded finished playbooks as unfinished; body unchanged). Type-scale floor raised across 23 metadata/description styles (0.62-0.74rem became 0.68-0.8rem; per-selector script with exactly-once asserts). Deadline badges read "closes today / closes tomorrow / closes in N days" (previously "deadline in 0 days" and a plural bug at 1 day), and the Conferences and Opportunities intros state that deadline dates are the organizer's own time zone. Homepage video grid has a per-channel cap (feeds.yaml video_feeds.home_max_per_channel, default 2, top-up from newest remaining; aggregate.diversify_by_source), so one channel's launch-day burst cannot fill the grid; the Videos page is uncapped. Pages enrolled in prose page review render a "Content last reviewed" footer from their front matter (render_data hook, markdown link so MkDocs validates it; About's automation section explains the mechanism and the single-field pipeline exception). The news category pages' standing line now says summaries are machine generated (aggregate.selection_note). docs/faculty.md (For Faculty & Staff) is the faculty twin of the students page: routing-only, task-based, grounded solely in existing pages and the policy; top-level nav before For Students; it replaces the Playbooks homepage card so the audience cards pair. Nav collapse implemented 2026-07-09 after owner discussion (labels are the owner's picks): eight top-level sections, Home | Learn | Use AI | For Faculty & Staff | For Students | News & Events | Governance | About. Use AI absorbs the tool guides, Prompts, and Playbooks and lands on the Tool Directory; News & Events absorbs News, Benchmarks, Conferences, and Opportunities and lands on This Week; About absorbs Announcements. Both audience tabs and Governance deliberately stay top-level (first-visit legibility and institutional visibility), and the queued sections have designated homes (student FAQ under For Students; Local Voices top-level or under News & Events). URLs unchanged; the 14-tab spacing compression in extra.css was removed the same day it was added, since eight tabs fit at Material's default spacing down to the 1220px tab breakpoint (verified: 741px used at a 1280px viewport). Rejected: "Office of AI in Medical Education" header attribution (conflicts with the authority register) and a named-tool "Start here" list in the directory (endorsement ahead of ratification; a task-router remains open as an alternative). Owner-side: seeding the Prompt Exchange under his own name, the custom domain, and designating whether the policy page is the authoritative publication.
- Research Methods course adaptations (owner approved 2026-07-10; source: the owner's redesigned Research Methods 1 graduate course, blueprint in his separate course project; adapt-not-adopt by design, since the Hub's scope is AI and the course's decks and statistics content stay course-internal): three additions. (1) Three research prompts derived from the course's frameworks, all Draft until owner testing: Research question coach (Bordage typology, PICO/PICOT, SPIDER, FINER screen; hands the user the novelty search to run in Scopus rather than certifying novelty), Pre-submission reviewer (the documented Bordage 2001 rejection drivers as an R1-R10 audit with quote-anchored verdicts), Reporting guideline auditor (EQUATOR checklist completeness map with a design-gap-vs-writing-gap split; notes tell users to file the official checklist from equator-network.org at submission). (2) docs/playbooks/literature-reviews.md (Reviewing the Literature, faculty and graduate students): the seven-step question-to-appraised-evidence workflow, Scopus AI orientation then explicit recorded searches then seed-growing then conservative AI screening then hand appraisal (MERSQI, reporting checklists) then own-set synthesis then per-citation source verification; appraisal explicitly does not delegate. Nav under Use AI > Playbooks; playbooks index updated (literature reviews leaves the planned list); faculty page research route links it. (3) "Judged by the same rulers" section closing the AI for Research guide: EQUATOR, MERSQI, and ICMJE anchors for AI-assisted work (ICMJE provisions verified against the live recommendations 2026-07-10: no AI authorship, writing assistance disclosed in acknowledgments, AI in data collection or analysis in methods, human responsibility). The reverse integration (AI woven into the course itself, Scopus in Sessions 2 and 7, an AI-use course policy from the syllabus-statement playbook) was recommended to the owner the same day and is a course-side decision outside this SPEC.
- Choosing Your Interface guide (owner approved 2026-07-13; docs/tools/interfaces.md, nav under Use AI after AI Agents): the interface-selection and execution-economics page no peer site has. Three tiers (web chat; working sessions, Cowork and ChatGPT Work; code agents, Claude Code and Codex mode) with a comparison table; a tool-call explainer with an SVG loop figure; the honest economics of local tool execution (deterministic work on the CPU instead of regenerated output, read-what-you-need context economy, near-free verification loops); effort dials on both ecosystems (Claude low through max plus fast mode and ultracode; GPT-5.6 none through max plus ultra mode); a dated model-lineup snapshot (Fable 5, Opus 4.8, Sonnet 5; GPT-5.6 Sol/Terra/Luna, launched and merged into the ChatGPT desktop app 2026-07-09) that defers standings to Benchmarks; and an Equipping the Machine section whose core teaching is that the agent names its own toolchain, with a seven-tool starter table (LibreOffice, Poppler, Pandoc, Python, Node, optional Tesseract and ImageMagick) carrying winget commands verified on the owner's machine and brew commands verified against formulae.brew.sh the day of writing. Every product claim was verified by a four-agent workflow against vendor pages 2026-07-13; Anthropic facts are first-party (code.claude.com docs), OpenAI facts are cross-checked coverage because openai.com and help.openai.com 403 scripted clients, and single-sourced items (exact ultra-mode plan matrix, context windows) were hedged or omitted. Pricing numbers deliberately omitted (fastest-rotting fact class); the page opens by disambiguating local tools from local models. Deliberately the most volatile page on the site: the page-review census counts its model names and assigns it the floor review budget automatically, which is the designed mitigation.
...
- Per-task provider wiring shipped 2026-06-11 (owner spend plan, ANTHROPIC_API_KEY secret in place with prepaid credits, auto-reload off, and a workspace monthly limit): the feeds.yaml `llm.tasks` block assigns curation to claude-haiku-4-5, weekly digest selection to claude-sonnet-5 (upgraded from Haiku 2026-07-02, owner approved: one call per week makes the premium pennies a month, and the digest is the flagship emailed artifact), and the digest narrative to claude-fable-5 with claude-opus-4-8 as an in-request refusal fallback (owner editorial decision 2026-07-04, made knowing LiveBench has Opus slightly ahead globally: Fable writes denser short-form prose and Opus's tendency to expound works against the 320-word ceiling; ~13 cents a week vs ~6; the W27 narrative is Opus and W28 onward is Fable, a natural side-by-side), with section briefs staying on free gpt-4.1; every Anthropic task falls back to the free GitHub path, then keyword mode, when the key is absent or a call fails. Verified live in CI: curation mode reports `llm (anthropic, claude-haiku-4-5)`. Standing decision 2026-07-02: curation stays on Haiku until the drop-reason logs (the audit trail above) show whether its misfires are rule misapplication, which would justify Sonnet, or defensible judgment calls, which would not. The review ran 2026-07-07 (owner authorized, one day past the planned 2026-07-06) over roughly 130 drop reasons from the 14 refresh runs since the audit trail shipped. Verdict: **curation stays on claude-haiku-4-5.** The overwhelming majority of drops cited rules that plainly fit (the preprint bar on arXiv floods, vendor marketing, rumor-framed titles, off-topic clinical papers); no pattern of rule misapplication. The one systematic finding was input starvation, not model weakness: several drops said "incomplete candidate data" because per-candidate truncation (140-character titles, 100-character summaries, sized for the free tier's 8,000-token cap) cut items mid-sentence. Fixed the same day: `_pack_candidates` truncation is provider-configurable and the Anthropic block carries `candidate_title_chars: 200` / `candidate_summary_chars: 400` (roughly a cent a day more). Re-open the model question only if a future audit shows genuine rule misapplication; if upgrading then, claude-sonnet-4-6 is a candidate alongside claude-sonnet-5: on the site's own LiveBench snapshot (2026-01-08 release, read 2026-07-03) Sonnet 4.6 at medium effort matches Sonnet 5 at xhigh effort globally (75.5 vs 75.8) and beats it on Instruction Following (63.2 vs 59.4), the dimension these contract-following tasks live on. Second decision point, before 2026-09-01: re-evaluate sonnet-5 versus sonnet-4-6 for digest_selection and content_watch on their actual outputs (W28 digest onward), because Sonnet 5's introductory pricing lapses 2026-08-31 and its newer tokenizer counts roughly 30% more tokens for the same text, making 4.6 effectively ~20-25% cheaper per identical call from September while Sonnet 5 is effectively ~13% cheaper until then.
