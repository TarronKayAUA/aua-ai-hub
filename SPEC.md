...
- The community Prompt Exchange is a Discussions category (node id pinned in the `community:` block of feeds.yaml so renames are safe). The nightly pipeline queries the GraphQL API for the category's discussions, sorts by upvotes, and renders the top N into `includes/community-prompts.md` (generated; keep-last-good on failure), which the Prompts page pulls in under a not-yet-reviewed framing. Community prompts enter the reviewed library only through owner testing and promotion, with contributor credit. A pinned welcome post documents the sharing format and ground rules (no PHI, FERPA data, or exam content).
- Moderation happens through GitHub's Discussions tools under the owner's account.
