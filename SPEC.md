...
- The image and video pages each carry a nightly arena snapshot generated into `includes/media-image.md` and `includes/media-video.md` from the Artificial Analysis free Data API (`media_arenas` block in feeds.yaml; endpoints `/api/v2/media/<arena>/models`; attribution stated under each table). The key exists only as the `ARTIFICIAL_ANALYSIS_LEADERBOARD_API` repository secret; without it the pipeline skips the step and says so in the verification block, and on any fetch or parse failure the previous snapshot is kept, mirroring the LiveBench behavior.
