...
Curated artificial intelligence reference and news hub for the American University of Antigua College of Medicine (AUACOM). Maintained by Dr. Tarron Kayalackakom, Associate Dean of AI in Medical Education.
...
the newest digest narrative carry audio players generated by
`scripts/narrate.py`. Two voices, both configured in the `narration` block
of `feeds.yaml`: static pages use a metered cloud voice (Speechify), the
news briefs use the local Kokoro model.

**Editing a narrated static page now requires a local step.** The metered
key is deliberately local only and is never a GitHub secret, so CI cannot
re-read a page for you. If you edit one and do not regenerate, the build
serves that page with no player at all, rather than reading superseded
words aloud, and the daily narration-health workflow opens an issue naming
the pages. To fix, run

    python scripts/narrate.py --only static

and commit the changed `.mp3` and `.sha256` under `docs/assets/audio/`
together. Use `--only static` rather than a bare run, which would also try
to build the Kokoro engine for the news briefs. `--dry-run` shows what
would regenerate and what it would cost; `--check` exits non-zero if
anything is stale; `--voices` lists the voices the configured model
supports, with preview clips, without generating anything.

Metered spend is tracked in `data/narration_spend.json` against
`narration.monthly_char_budget` in `feeds.yaml`, and a run that would
exceed the remaining allowance refuses to start rather than stopping part
way. If that ledger is ever unreadable the guard refuses too, because an
unknown month must not be treated as an unspent one. Mispronunciations:
add the term under the right engine in `data/narration_lexicon.yaml`; a
substitution that rescues one voice usually harms another, so the tables
are per engine.
...
**Never hand-edit** `docs/news/**`, `docs/assets/audio/news-*` and `digest-*`, `data/narration_spend.json`, `docs/prompts/exchange.md`,
