...
**Narrate a page (read-aloud players)** — the seven pathway modules, the
news section briefs, and the newest digest narrative carry audio players
generated by `scripts/narrate.py` (Kokoro voice, no API). Every CI build
re-reads any narrated page whose text changed, and the refresh workflow
commits the new static audio back, so text and audio cannot drift on the
site. To make an edit's audio durable immediately, run
`python scripts/narrate.py` in an environment with
`requirements-narration.txt` (Python 3.10 to 3.13; the model downloads
once) and commit the changed `.mp3` and `.sha256` under
`docs/assets/audio/` together; `--dry-run` shows what would regenerate.
Mispronunciations: add the term to `data/narration_lexicon.yaml`.
...
**Never hand-edit** `docs/news/**`, `docs/assets/audio/news-*` and `digest-*`, `docs/prompts/exchange.md`,
