...
- Owner-owned data files: data/tools.yaml, data/open_models.yaml, data/conferences.yaml, data/conference_watchlist.yaml, data/prompts.yaml, data/prompt_resources.yaml, data/committee.yaml, data/committee_work.yaml, data/polls.yaml, feeds.yaml, prompts/curator.md, prompts/digest.md. graphics/ holds official AUA logos and member photos; regenerate web assets with scripts/build_brand_assets.py. The approved AI policy on docs/governance/policy.md is verbatim institutional text: never edit its wording, only replace it whole when the owner supplies a newly approved version.
