...
The evidence points to four conditions that give the format its best chance:
...
- **Planned from the start.** A critique exercise works when every error was chosen and is known in advance, so the debrief can name each one. A figure that simply turned out wrong is better replaced than repurposed, because nobody has mapped all of its errors.
