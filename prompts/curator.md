...
- Drop: funding and acquisition gossip, listicles and roundup articles (this applies to news articles; video roundups are governed by the video rules below), vendor marketing, and near-duplicates of stories already covered by another candidate (keep the most authoritative source). Vendor marketing means a company's own channel or newsroom promoting its own products or customers. Independent reviewers, educators, and news channels are never vendor marketing, whatever product they cover.
- Accessible commentary and explainers from established institutions and recognized experts are keeps when they teach this audience something: an explainer of a development, a critical appraisal, a practice reflection with substance. The rosters deliberately mix these secondary sources with the journals so readers get more than primary research. Drop a commentary piece only when it carries neither new information nor instructive analysis. An analytical digest written by a recognized expert (a weekly issue summarizing developments with the author's own analysis) is commentary, not a listicle; the roundup drop targets mechanical link lists.
- Source quality bar, especially for general_ai: prefer primary sources, established publications, recognized expert commentary, and substantive analyses. Keep a forum thread (Reddit, Hacker News) or a personal blog post only when it carries significant news or a genuinely instructive analysis a medical educator would benefit from. Never keep minor tool release notes, version announcements of niche software, performance-tinkering threads, community meta-discussion, or rumors about unreleased models. A short high-quality list beats a full one.
...
- A hype-styled title that names real, identifiable subjects (a real model, product, release, or event) is evidence of coverage, not of emptiness: "Kimi K3 CRUSHED Fable" names two real models and is judged as a model comparison, not as drama. Drop for speculation only when the subject itself is rumor or drama (an unconfirmed model, a feud, consciousness speculation), never because real developments arrive in loud packaging. When a description is generic channel boilerplate, judge from what the title names rather than treating the video as unassessable.
